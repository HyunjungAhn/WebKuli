...
public class ConTestResponder implements SecureResponder {
	...
	protected String createPageContent() throws Exception {		
		...
		String contestProp = pageData.getVariable(TestSystem.CONTEST_PROP);
		if (null == contestProp) {
			throw new ConTestDefinitionException();
		}
		reader = new FileReader(contestProp);
	...
	private String doMakeHtml(String resource, FitNesseContext context,
			boolean firstTimeForNewPage) throws Exception {
		HtmlPage html = context.htmlPageFactory.newPage();
		String title = firstTimeForNewPage ? "Page doesn't exist. Edit "
				: "Edit ";
		html.title.use(title + resource + ":");
		html.body.addAttribute("onload", "document.f." + CONTENT_INPUT_NAME
				+ ".focus()");
		HtmlTag header = makeHeader(resource, title);
		html.header.use(header);
		html.main.use(makeEditForm(resource, firstTimeForNewPage,
				context.defaultNewPageContent));
	...
	private HtmlTag makeEditForm(String resource, boolean firstTimeForNewPage,
			String defaultNewPageContent) throws Exception {
		HtmlTag form = new HtmlTag("form");
		form.addAttribute("name", "f");
		form.addAttribute("action", resource);
		form.addAttribute("method", "post");
		form.add(HtmlUtil.makeInputTag("hidden", "responder", "saveContest"));
		form.add(HtmlUtil.makeInputTag("hidden", TIME_STAMP, String
				.valueOf(SaveRecorder.timeStamp())));
		form.add(HtmlUtil.makeInputTag("hidden", TICKET_ID, String
				.valueOf((SaveRecorder.newTicket()))));
		if (request.hasInput("redirectToReferer")
	...
	private HtmlTag makeScriptOptions() {
		...
		includeJavaScriptFile("/files/javascript/textareaWrapSupport.js",
				scripts);
	...
	private HtmlTag createTextarea(boolean firstTimeForNewPage,
		...
		textarea.add(Utils
				.escapeHTML(firstTimeForNewPage ? defaultNewPageContent
						: content));
		return textarea;
	}
