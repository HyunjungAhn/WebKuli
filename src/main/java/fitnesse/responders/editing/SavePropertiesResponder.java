...
public class SavePropertiesResponder implements SecureResponder {
  ...
  private void saveAttributes(Request request, PageData data) throws Exception {
    ...
    attrs.addAll(Arrays.asList(PageData.CONCURRENCY_ATTRIBUTES));
    attrs.addAll(Arrays.asList(PageData.WEBDRIVER_ATTRIBUTES));
    ...
    String parallelSuites = (String) request.getInput("ParallelSuites");
    data.setAttribute(PageData.PropertyPARALLELSUITES, parallelSuites);

    String helpText = (String) request.getInput("HelpText");
    data.setAttribute(PageData.PropertyHELP, helpText);
    
    String mobileDeviceAddress = (String) request.getInput("MobileDevice");
    
    data.setAttribute(WikiPageProperties.MOBILE_DEVICE_ATTRIBUTE, mobileDeviceAddress);
  }
