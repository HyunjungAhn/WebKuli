...
public class PropertiesResponder implements SecureResponder {
	...
	private static String rootDirectoryName = "FitNesseRoot";
	...
	private JSONObject makeJson() throws Exception {
		...
		String attributes[] = new String[] {TEST.toString(), PropertySEARCH, PropertyEDIT, "ConTest", PropertyLASTROWTEST, PropertyPROPERTIES, PropertyVERSIONS, PropertyREFACTOR, PropertyWHERE_USED, PropertyRECENT_CHANGES, SUITE.toString(), PARALLELSUITE.toString(), PropertyPRUNE, PropertySECURE_READ, PropertySECURE_WRITE, PropertySECURE_TEST};
	...
	private HtmlTag makePropertiesForm() throws Exception {
		...
		checkBoxesSection.add(makeConcurrencyCheckboxesHtml(pageData));
		checkBoxesSection.add(makeWebdriverCheckboxesHtml(pageData));
		...
		HtmlTag mobileWikiSection = new HtmlTag("div");
		mobileWikiSection.addAttribute("class", "mobile-wiki-properties");
		mobileWikiSection.add(makeMobileDeviceAddressHtml());

		trisection.add(checkBoxesSection);
		trisection.add(virtualWikiSection);
		trisection.add(mobileWikiSection);
		form.add(trisection);
	...
	private HtmlTag makeMobileDeviceAddressHtml() throws Exception {
		HtmlTag mobileDevice = new HtmlTag("div");
		mobileDevice.addAttribute("style", "float: left; width: 350px;");
		mobileDevice.add("Mobile Device Address: ");
		mobileDevice.add(HtmlUtil.BR);
		HtmlTag vwInput = HtmlUtil.makeInputTag("text", "MobileDevice", getMobileDeviceValue(pageData, false));
		vwInput.addAttribute("size", "100");
		mobileDevice.add(vwInput);
		mobileDevice.add(HtmlUtil.NBSP);
		mobileDevice.add(HtmlUtil.NBSP);
		return mobileDevice;
	}
	...
	public static String getMobileDeviceValue(PageData data, boolean bFindParent) throws Exception {
		WikiPage currentWikiPage = data.getWikiPage();
		if (currentWikiPage.getName().equals(rootDirectoryName))
			return "";

		String value = data.getAttribute(WikiPageProperties.MOBILE_DEVICE_ATTRIBUTE);
		if (value == null) {
			if (bFindParent == false)
				return "";

			WikiPage parentWikiPage = currentWikiPage.getParent();
			PageData parentData = parentWikiPage.getData();
			return getMobileDeviceValue(parentData, true);
		}
		return value;
	}
	...
	public HtmlTag makeConcurrencyCheckboxesHtml(PageData pageData) throws Exception {
		return makeAttributeCheckboxesHtml("Concurrent Testing:", PageData.CONCURRENCY_ATTRIBUTES, pageData);
	}

	public HtmlTag makeWebdriverCheckboxesHtml(PageData pageData) throws Exception {
		return makeAttributeCheckboxesHtml("WebDriver:", PageData.WEBDRIVER_ATTRIBUTES, pageData);
	}
	...
	public HtmlTag makeTagsHtml(PageData pageData) throws Exception {
		...
		div.add(makeInputField("Tags:", PropertyPARALLELSUITES, PropertyPARALLELSUITES, 40, pageData));
		return div;
	}
