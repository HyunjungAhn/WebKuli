...
public abstract class BaseWikiPage implements WikiPage {
  ...
  private List<WikiPageAction> makeActions(String localPageName, String localOrRemotePageName, boolean newWindowIfRemote) throws Exception {
    ...
    addActionForAttribute("Smart", pageData, localPageName, newWindowIfRemote, null, null, actions);
    addActionForAttribute("LastRowTest", pageData, localPageName, newWindowIfRemote, null, null, actions);
    addActionForAttribute("Suite", pageData, localPageName, newWindowIfRemote, "", null, actions);
    addActionForAttribute("ParallelSuite", pageData, localPageName, newWindowIfRemote, "", null, actions);
    addActionForAttribute("Edit", pageData, localOrRemotePageName, newWindowIfRemote, null, null, actions);
	addActionForAttribute("ConTest", pageData, localOrRemotePageName, newWindowIfRemote, "", null, actions);
	addActionForAttribute("SmartBox", pageData, localOrRemotePageName, newWindowIfRemote, "", null, actions);
  ...
  private void addActionForAttribute(String attribute, PageData pageData, String pageName, boolean newWindowIfRemote,
    ...
    if (pageData.hasAttribute(attribute.replaceAll("\\s", ""))) {
	  if (attribute.compareToIgnoreCase("contest") == 0) {
		fit.FitServer.enableContest = true;
		addAction(attribute, pageName, newWindowIfRemote, shortcutKey, query, actions);
	  } else if (attribute.compareToIgnoreCase("smartbox") == 0) {
		fit.FitServer.enableSmartBox = true;
	  } else {
		addAction(attribute, pageName, newWindowIfRemote, shortcutKey, query, actions);
	  }
    } else if (attribute.compareToIgnoreCase("contest") == 0) {
		fit.FitServer.enableContest = false;
		fit.FitServer.enableSmartBox = false;
	}
  }
