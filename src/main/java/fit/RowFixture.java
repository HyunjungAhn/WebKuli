...
abstract public class RowFixture extends ColumnFixture {
	...
	private static final String ALL_EXTENSION_IMAGE_PATTERN = "(\"([\\w\\S]+/([\\w]+\\.(jpg|gif|png|bmp|JPG|GIF|PNG|BMP)))\")+";
	private static final String WRONG_IMAGE_SIZE = "-1x-1";
		...
		if (col >= columnBindings.length) {
			...
			for (Iterator<Object> i = keys.iterator(); i.hasNext();) {
				...
				if (eList == null) {
					...
					if ("displayonly".equals(key.toString().toLowerCase())) {
						isOnlyDisplay = true;
					} else {
	...
	protected Map<Object, Object> eSort(List<?> list, int col) {
		...
		for (Iterator<?> i = list.iterator(); i.hasNext();) {
			...
			try {
				...
				if (FitServer.hasSymbol(text)) {
					String value = Fixture.replaceSymbol(text);

					if (null != value) {
						text = value;
					}
				}
				Object key = a.parse(text);
				bin(result, key, row);
			} catch (Exception e) {
				exception(cell, e);
	...
	public void wrong(String value, Parse cell) {
		counts.wrong++;
		cell.body = "<div class=fail>" + value + "</div>";
	}
	...
	protected Parse buildCells(Object row) {
		...
		for (int i = 0; i < columnBindings.length; i++) {
			...
			if (a == null) {
				...
				try {
					a.target = row;
					String value = a.toString(a.get());
					
					if (true == isOnlyDisplay && i == 0) {
						Pattern pattern = Pattern.compile(ALL_EXTENSION_IMAGE_PATTERN);
						Matcher matcher = pattern.matcher(value); 
						if (true == matcher.find()) {
							next.body = gray(("<img src=\"" + value + "\"/>"));
						} else {
							next.body = gray(value);
						}
					} else {
						next.body = gray(escape(value));
						if (WRONG_IMAGE_SIZE.equals(value)) {
							wrong(value, next);
						}
					}
				} catch (Exception e) {
